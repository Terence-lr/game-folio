...
export function useFishMovement() {
  ...
  const [fishRotation, setFishRotation] = useState(new THREE.Euler(0, 0, 0));
  ...
  // Trigger boost from mobile controls
  const triggerBoost = useCallback((boosting: boolean) => {
    setIsBoosting(boosting);
    console.log('🚀 Fish boost:', boosting);
  }, []);

  // Update fish movement
  const updateFishMovement = useCallback((delta: number) => {
    const keys = getKeys();
  ...
  }, [fishPosition, fishRotation, mobileInput, isBoosting, getKeys]);
